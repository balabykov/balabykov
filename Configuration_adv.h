...
#define LIN_ADVANCE
  ...
  #define LIN_ADVANCE_K 0.03    // Unit: mm compression per 1mm/s extruder speed (typical for ~90% of filaments; re-tune per spool with M900 K)
...
/**
 * === PORTED FORWARD FOR MARLIN 2.1.2.8 ===
 * This feature (ZV Input Shaping) does not exist in this old 2.0.1.1 codebase at all —
 * it was added to Marlin in version 2.1.2. These lines will do nothing here; they are a
 * placeholder to paste into the real Configuration_adv.h once building against the actual
 * 2.1.2.8 source tree. Confirmed on-device values from the printer's Advanced Settings
 * menu (old firmware, before the Y bed was swapped to carbon):
 *   X frequency 30.0 Hz / damping 0.60  -> X axis unchanged, likely still valid
 *   Y frequency 28.3 Hz / damping 0.60  -> Y bed swapped metal->carbon (-900g), NEEDS RECALIBRATION
 * See https://marlinfw.org/docs/features/input_shaping.html
 */
#define INPUT_SHAPING_X
#if ENABLED(INPUT_SHAPING_X)
  #define SHAPING_FREQ_X  30.0   // (Hz) X axis unchanged since old calibration - carry over
  #define SHAPING_ZETA_X  0.60   // Damping ratio
#endif

#define INPUT_SHAPING_Y
#if ENABLED(INPUT_SHAPING_Y)
  #define SHAPING_FREQ_Y  28.3   // (Hz) TEMPORARY - Y bed changed (carbon, -900g), recalibrate after install
  #define SHAPING_ZETA_Y  0.60   // Damping ratio
#endif
